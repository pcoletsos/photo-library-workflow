<#
.SYNOPSIS
    Previews the renaming of child folders by adding a "YYYY-MM" prefix based on the oldest file's date.

.DESCRIPTION
    This script inspects all files within each subfolder (including hidden files) to find the oldest date.
    It uses "Date Taken" first, falling back to the file's creation date if necessary. It then shows which
    folders it would rename with a "YYYY-MM - " prefix. It will not re-process folders that already have this prefix.
...
Write-Host "Mode: Adding 'YYYY-MM -' prefix based on oldest file (SAFE MODE)." -ForegroundColor Yellow
